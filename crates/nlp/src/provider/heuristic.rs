...
use super::{
    AiProvider, Capabilities, ExtractContext, Extraction, Intent, Operation, ProviderError,
};
...
static QUESTION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"\?|^what|^how|^why|^when|^where|^who|^which|^can\b|^should\b|^could\b|^is\b|^are\b",
    )
    .unwrap()
});
static TASK_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\bi need to\b|\bi should\b|\bwe need to\b|\btodo:|\btodo\b|\bdon'?t forget\b|\bnext step\b|\bfollow up\b").unwrap()
});
static DECISION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\blet'?s\b|\bwe'?ll\b|\bdecided\b|\bgoing with\b|\bgo with\b|\bchose\b").unwrap()
});
static OPINION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\bi think\b|\bi feel\b|\bimo\b|\bin my opinion\b|\bi prefer\b|\bi like\b|\bi don'?t like\b").unwrap()
});
static UPDATE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\bactually\b|\bnow\b|\bswitched\b|\bchanged\b|\bupdated\b|\bmoved to\b|\bis now\b|\bare now\b").unwrap()
});

// Operation patterns.
static ADD_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\badd(?:ed|ing)?\b|\bappend(?:ed|ing)?\b|\binclude(?:d|ing)?\b").unwrap()
});
static REMOVE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"\bremove(?:d|ing)?\b|\bdelete(?:d|ing)?\b|\bdrop(?:ped|ping)?\b|\bcross(?:ed|ing)? off\b",
    )
    .unwrap()
});
static REPLACE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\bswitch(?:ed|ing)?\b|\bchang(?:ed|ing)?\b|\breplaced?\b|\bmoved? to\b|\bis now\b|\bare now\b").unwrap()
});
static CONTRADICT_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\bactually\b|\bcorrection\b|\bwrong\b|\bnot true\b|\bnope\b").unwrap()
});
...
mod tests {
    ...
    async fn smoke_test_supersede_turns() {
        ...
        assert!(
            matches!(
                t2.operation,
                Some(Operation::Replace) | Some(Operation::Contradict)
            ),
            "t2 op: {:?}",
            t2.operation
        );
    ...
    async fn classifies_questions() {
        ...
        let ctx = ExtractContext::default();
        let r = p
            .extract("What deploy target are we using?", &ctx)
            .await
            .unwrap();
    ...
    async fn classifies_tasks() {
        ...
        let ctx = ExtractContext::default();
        let r = p
            .extract("I need to update the rate limiter config.", &ctx)
            .await
            .unwrap();
